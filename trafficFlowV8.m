...
% Version 8 fixes user input road layout functionality, adds an additional
% preset with two intersections, and refactors car movement into a
% function.

clear; clc; close all;

% Inputs
preset = input("Choose a preset (1=Cross Intersection, 2=Two Intersections, 0=Custom): ");

if preset == 0
    % Custom user input
    fprintf('Creating a custom road layout:\n');
    gridSize = input("Enter grid size [rows cols] (minimum 20x20 recommended): ");
    
    % Validate grid size
    if length(gridSize) ~= 2 || gridSize(1) < 10 || gridSize(2) < 10
        fprintf('Invalid grid size. Setting to default 30x30.\n');
        gridSize = [30 30];
    end
    
    numCarsPerMin = input("Number of cars per minute (for each direction): ");
    if numCarsPerMin <= 0
        fprintf('Invalid car rate. Setting to default 20 cars/min.\n');
        numCarsPerMin = 20;
    end
    
    simTime = input("Simulation time (seconds): ");
    if simTime <= 0
        fprintf('Invalid simulation time. Setting to default 120 seconds.\n');
        simTime = 120;
    end
    ...
    fprintf('\nDefine your road layout row by row\n');
    fprintf('Use the following codes:\n');
    fprintf('0 = No road\n');
    fprintf('1 = Horizontal road (east-west)\n');
    fprintf('2 = Vertical road (north-south)\n');
    fprintf('3 = Intersection\n\n');
    
    for i = 1:gridSize(1)
        % Create a default string of zeros for this row
        defaultRow = repmat('0', 1, gridSize(2));
        fprintf('Row %d default: %s\n', i, defaultRow);
        
        rowInput = input(['Enter row ', num2str(i), ' (or press Enter for default): '], 's');
        
        % If user just pressed Enter, use the default row
        if isempty(rowInput)
            rowInput = defaultRow;
        end
        
        % Pad or truncate the input to match grid width
        if length(rowInput) < gridSize(2)
            rowInput = [rowInput, repmat('0', 1, gridSize(2) - length(rowInput))];
        elseif length(rowInput) > gridSize(2)
            rowInput = rowInput(1:gridSize(2));
        end
        ...
        for j = 1:gridSize(2)
            ...
            if val >= 0 && val <= 3
                ...
                % Invalid input defaults to no road
                roadGrid(i, j) = 0;
                roadDirections(i, j) = 0;
            end
        end
        
        % Show the processed row
        rowStr = '';
        for j = 1:gridSize(2)
            rowStr = [rowStr, num2str(roadGrid(i, j))];
        end
        fprintf('Row %d set to: %s\n', i, rowStr);
    end
    
elseif preset == 1
    % Default preset with cross intersection
    fprintf('Loading preset 1: Single Cross Intersection\n');
    gridSize = [30 30];
    numCarsPerMin = 20;
    simTime = 120;
    carSpawnRate = numCarsPerMin / 60;
...
elseif preset == 2
    % New preset with two intersections
    fprintf('Loading preset 2: Two Intersections\n');
    gridSize = [30 40];
    numCarsPerMin = 15;
    simTime = 180;
    carSpawnRate = numCarsPerMin / 60;

    % Create road grid with two intersections
    roadGrid = zeros(gridSize);
    roadDirections = zeros(gridSize);
    
    % Center points for the two intersections
    center1Row = ceil(gridSize(1)/2);
    center1Col = ceil(gridSize(2)/3);
    
    center2Row = ceil(gridSize(1)/2);
    center2Col = ceil(2*gridSize(2)/3);
    
    % Horizontal road (two lanes - two-way road)
    roadGrid(center1Row-1:center1Row, :) = 1; % Two lanes
    roadDirections(center1Row-1:center1Row, :) = 1; % Two lanes
    
    % Vertical roads (two lanes - two-way road)
    roadGrid(:, center1Col-1:center1Col) = 2; % First vertical road
    roadDirections(:, center1Col-1:center1Col) = 2;
    
    roadGrid(:, center2Col-1:center2Col) = 2; % Second vertical road  
    roadDirections(:, center2Col-1:center2Col) = 2;
    
    % Mark intersections
    roadGrid(center1Row-1:center1Row, center1Col-1:center1Col) = 3; % First intersection
    roadDirections(center1Row-1:center1Row, center1Col-1:center1Col) = 3;
    
    roadGrid(center1Row-1:center1Row, center2Col-1:center2Col) = 3; % Second intersection
    roadDirections(center1Row-1:center1Row, center2Col-1:center2Col) = 3;
    
else
    % Default to preset 1 if invalid choice
    fprintf('Invalid preset choice. Defaulting to preset 1: Single Cross Intersection\n');
    gridSize = [30 30];
    numCarsPerMin = 20;
    simTime = 120;
    carSpawnRate = numCarsPerMin / 60;
        ...
        if clusterIDs(i) == 0 % Unassigned intersection
            ...
            for j = 1:numIntersections
                if i ~= j && clusterIDs(j) == 0
                    dist = sqrt((intersectRows(i) - intersectRows(j))^2 + (intersectCols(i) - intersectCols(j))^2);
...
for t = 1:simTime
    ...
    % Move cars using the moveCars function
    [newGrid, newCarIDs, carPassedIntersection, numPassed] = moveCars(grid, carIDs, allCars, roadGrid, gridSize, directionOffsets, minCarSpacing, stopLinePositions, lightStates, lightPositions, intersectionZones, carPassedIntersection, numIntersections);
    
    % Update counters
    passedCars = passedCars + numPassed;
    
    % Update the grid for the next iteration
    grid = newGrid;
    carIDs = newCarIDs;
    
    % Spawn new cars with spawnCars function
    [grid, carIDs, totalCarsAdded] = spawnCars(grid, carIDs, roadGrid, gridSize, carSpawnRate, minCarSpacing, nextCarID);
    
    % Update counters and nextCarID
    totalCars = totalCars + totalCarsAdded;
    nextCarID = nextCarID + totalCarsAdded;
    
    % Update visual grid with cars and check for collisions
    [visualGrid, collisionCount] = updateVisualGrid(grid, visualGrid, gridSize);
    ...
    statusStr = sprintf('Time: %d/%d sec | Cars: %d | Passed: %d (%.1f%%) | Collisions: %d | %s', t, simTime, totalCars, passedCars, efficiency, collisions, lightStatus);
    title(statusStr);
    
    drawnow;
    disp(grid)
    pause(0.5); % Animation speed
end
...
function [newGrid, newCarIDs, carPassedIntersection, numPassed] = moveCars(grid, carIDs, allCars, roadGrid, gridSize, directionOffsets, minCarSpacing, stopLinePositions, lightStates, lightPositions, intersectionZones, carPassedIntersection, numIntersections)
    ...
    % Counter for cars that have passed through the grid
    numPassed = 0;
    ...
    for i = 1:size(allCars, 1)
        ...
        if nr >= 1 && nr <= gridSize(1) && nc >= 1 && nc <= gridSize(2) && roadGrid(nr, nc) > 0
            ...
            for sl = 1:size(stopLinePositions, 1)
                ...
                if r == stopR && c == stopC
                    ...
                    if ~isempty(carPassedIntersection)
                        carPassedThisIntersection = any(carPassedIntersection(:, 1) == carID & carPassedIntersection(:, 2) == stopIntersectionIdx);
                    end
                    ...
                    if ~carPassedThisIntersection
                        ...
                        for lp = 1:size(lightPositions, 1)
                            if lp <= size(lightPositions, 1) && stopIntersectionIdx <= numIntersections && lightPositions(lp, 4) == stopIntersectionIdx && lightPositions(lp, 3) == stopDir
            ...
            if nextIsIntersection && intersectionZones(nr, nc) > 0
                ...
                carPassedThisIntersection = false;
                if ~isempty(carPassedIntersection)
                    carPassedThisIntersection = any(carPassedIntersection(:, 1) == carID & carPassedIntersection(:, 2) == intersectionID);
                end
                    ...
                    if occupiedPositions(moveRow, moveCol) == 0
                        ...
                        if occupiedPositions(r, c) == 0
                            ...
                            for dist = 1:3  % Try up to 3 cells back
                                ...
                                if backR >= 1 && backR <= gridSize(1) && backC >= 1 && backC <= gridSize(2) && roadGrid(backR, backC) > 0 && occupiedPositions(backR, backC) == 0
...
function [grid, carIDs, totalCarsAdded] = spawnCars(grid, carIDs, roadGrid, gridSize,  carSpawnRate, minCarSpacing, nextCarID)
    % Find the middle of the grid for reference
    centerRow = ceil(gridSize(1)/2);
    centerCol = ceil(gridSize(2)/2);
    
    % Counter for new cars added
    totalCarsAdded = 0;
    ...
    for r = 1:gridSize(1)
        if roadGrid(r, 1) == 1 && mod(r - centerRow, 2) == 1 && rand < carSpawnRate && grid(r, 1) == 0
            ...
            if ~carInFront
                ...
                carIDs(r, 1) = nextCarID + totalCarsAdded;
                totalCarsAdded = totalCarsAdded + 1;
            end
        end
    end
    ...
    for r = 1:gridSize(1)
        if roadGrid(r, gridSize(2)) == 1 && mod(r - centerRow, 2) == 0 && rand < carSpawnRate && grid(r, gridSize(2)) == 0
            ...
            if ~carInFront
                ...
                carIDs(r, gridSize(2)) = nextCarID + totalCarsAdded;
                totalCarsAdded = totalCarsAdded + 1;
            end
        end
    end
    ...
    for c = 1:gridSize(2)
        if roadGrid(1, c) == 2 && mod(c - centerCol, 2) == 0 && rand < carSpawnRate && grid(1, c) == 0
            ...
            if ~carInFront
                ...
                carIDs(1, c) = nextCarID + totalCarsAdded;
                totalCarsAdded = totalCarsAdded + 1;
            end
        end
    end
    ...
    for c = 1:gridSize(2)
        if roadGrid(gridSize(1), c) == 2 && mod(c - centerCol, 2) == 1 && rand < carSpawnRate && grid(gridSize(1), c) == 0
            ...
            if ~carInFront
                ...
                carIDs(gridSize(1), c) = nextCarID + totalCarsAdded;
                totalCarsAdded = totalCarsAdded + 1;
            end
        end
    end
...
% Visual Grid Update Function
function [visualGrid, collisionCount] = updateVisualGrid(grid, visualGrid, gridSize)
